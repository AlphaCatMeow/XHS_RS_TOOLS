# Connections Notification 分页 payload 指南

本文档详细说明新增关注通知接口 (`/api/notification/connections`) 的分页机制。

## 接口信息

| 项目 | 值 |
|------|-----|
| 端点 | `GET /api/notification/connections` |
| 描述 | 获取新增关注通知列表 |
| 分页方式 | **游标分页** (`cursor`) |
| 每页数量 | **固定 20 条** |
...
### 第1页 (首次请求)
```
GET /api/notification/connections?num=20&cursor=
```

### 第2页
```
GET /api/notification/connections?num=20&cursor=7553158242479239810
```
...
def get_connections_notification(max_pages: int = 5):
    """获取新增关注通知分页示例"""
    all_messages = []
    cursor = ""  # 首次为空
    ...
    for page in range(1, max_pages + 1):
        ...
        url = f"{BASE_URL}/api/notification/connections?{urlencode(params)}"
...
if __name__ == "__main__":
    messages = get_connections_notification(max_pages=3)
